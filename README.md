...
Only rows under `/bahrain/bh-module` are counted — in the XP total, the project counts and all three
graphs — by the same `isModule` check, so every panel describes the same set of work. That prefix is
what the platform's own profile page totals: it keeps `piscine-js`, which sits inside the module, and
leaves out the separate `/bahrain/bh-piscine`.
